class FormTemplate < SourceAdapter
  def initialize(source) 
     @base = 'http://localhost:3001/form_templates'
     super(source)
  end
  ...
  def create(create_hash)
    result = RestClient.post(@base, :form => create_hash)
  ...
  def xmetadata
    ...
    objectlink = { :type => "linkli", :uri => "{{showuri}}", :text => "{{object}}-{{form_template_name}}" }
    list = { :type => "list", :children => [objectlink], :repeatable => "{{@forms}}" }
    content = { :type => "content", :children => [list] }
    toolbar = { :title => 'REMI TITLE', :type => "toolbar", :left_uri => "/app", :right_uri => "/app/Form/new", :left_text => "Test", :right_text => "New"}
    ...
    ##SHOW
    showformname = { :type => 'labeledvalueli',
      :label => 'Name',
      :value => '{{@form/name}}' }
    
    
    showlist = { :type => 'list',
      :children => [ showformname ] }
    
    
    showcontent = { :type => 'content',
      :children => [ showlist ] } 
    
    showtoolbar = { :title => '{{@form/name}}', 
      :type => 'toolbar',
      :left_uri => 'index',
      :left_text => 'Back',
      :left_class => 'backButton',
    ...
    edittoolbar = { :title => '{{@form/name}}', :type => 'toolbar',
      :left_uri => '/app/Form',
      :left_text => 'Cancel',
      :left_class => 'backButton',
      :right_uri => '{{@form/deleteuri}}',
      :right_text => 'Delete' }
